...
const { getAdminHtml } = require('../src/admin/ui');

async function testLocalHtml() {
  console.log('=== TESTING LOCAL FIXED getAdminHtml() IN PLAYWRIGHT ===');
  ...
  for (let i = 0; i < tabsToTest.length; i++) {
    ...
    tabMetrics.ancestors.forEach((a, idx) => {
      console.log(`  [${idx}] <${a.tagName} id="${a.id}" class="${a.className}"> display:${a.display} rect:[${a.rect.x},${a.rect.y},${a.rect.width},${a.rect.height}]`);
    });
  }
